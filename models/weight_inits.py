...
def _get_init_fn(init):
  ...
  if init_name == 'torch_default':
    ...
    if isinstance(gain, str):
      ...
      if len(init) > 2:
        param = init[2]
      gain = torch.nn.init.calculate_gain(gain, param)
...
def initialize_weights(model, user_weight_init={}):
  init_params = DEFAULT_INITS.copy()
  init_params.update(model.weight_init_params(user_weight_init))
  init_params.update(user_weight_init)
